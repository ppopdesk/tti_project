...
def get_results(res_path):
    ...
    # compute overall accuracy across all sources
    total_head_correct = 0
    total_tail_correct = 0
    total_examples = 0
    for k, (best_acc, head_correct, cnt, tail_correct) in res.items():
        total_head_correct += head_correct
        total_tail_correct += tail_correct
        total_examples += cnt

    if total_examples > 0:
        head_match_score = total_head_correct / total_examples
        tail_match_score = total_tail_correct / total_examples
        overall_acc = head_match_score if head_match_score > tail_match_score else tail_match_score
    else:
        overall_acc = 0.0

    # add an 'overall' entry to the result dict
    res['overall'] = [overall_acc, total_head_correct, total_examples, total_tail_correct]
...
if __name__ == "__main__":
    get_results('../../results/medqa_ft_eval/logs/tfmrmedqa_testl_api.json')