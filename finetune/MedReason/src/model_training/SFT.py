...
class Train_dataset(torch.utils.data.Dataset):
    def __init__(self, config, tokenizer):
        ...
        if self.config.base_model == 'Llama':
            ...
            chat_template = "{% set loop_messages = messages %}{% for message in loop_messages %}{% set content = '<|im_start|>' + message['role'] + '\n\n'+ message['content'] | trim + '<|eot_id|>' %}{% if loop.index0 == 0 %}{% set content =  content %}{% endif %}{{ content }}{% endfor %}{% if add_generation_prompt %}{{ '<|im_start|>assistant\n\n' }}{% endif %}"
    ...
    def get_response(self,da):
        if 'ours' in self.config.data_path:
            temp = '## Thinking\n\n{}\n\n## Final Answer\n\n{}'
            return temp.format(da['reasoning'],da['answer'])
        else:
            temp = '## Thinking\n\n{}\n\n## Final Response\n\n{}'
...
def train(args):
    ...
    # If launched with DeepSpeed, keep DS config in sync with CLI args. Otherwise, run plain Accelerate.
    if accelerator.state.deepspeed_plugin is not None:
        accelerator.state.deepspeed_plugin.deepspeed_config['train_micro_batch_size_per_gpu'] = args.train_bsz_per_gpu
        world_size = dist.get_world_size() if dist.is_available() and dist.is_initialized() else accelerator.num_processes
        accelerator.state.deepspeed_plugin.deepspeed_config['train_batch_size'] = (
            args.train_bsz_per_gpu * world_size * accelerator.gradient_accumulation_steps
        )

    tokenizer = AutoTokenizer.from_pretrained(args.model_path, trust_remote_code=True)

    # Reduce memory vs fp32 loading. For bf16 training this is typically what you want.
    # (Accelerate will still handle device placement/sharding depending on launch config.)
...
if __name__ == '__main__':
    ...
    # Experiment Args
    parser.add_argument('--experiment_name', type=str,default='Medreason')
    ...
    parser.add_argument(
        ...
        help='Optimizer. 8-bit variants reduce optimizer-state memory (recommended for 7B+ full finetune).',
    )
