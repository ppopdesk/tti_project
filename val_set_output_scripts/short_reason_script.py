...
def generate_cot_batch(
    ...
    prompts = [build_prompt(tokenizer, q, COT_PREFIX) for q in question_texts]
    ...
    rows = []
    for idx, ex, qtext, prompt, out in zip(batch_indices, batch_examples, question_texts, prompts, outputs):
        generated_text = out.outputs[0].text
        full_output = COT_PREFIX + generated_text
        gold_answer = str(ex["answer_idx"]).strip().upper()
...
def main():
    parser = argparse.ArgumentParser(description="Collect COT answers with vLLM and cache them.")
    ...
    for batch_start in range(0, len(dataset), args.batch_size):
        ...
        batch_indices = list(range(batch_start, min(batch_start + args.batch_size, len(dataset))))

        batch_rows = generate_cot_batch(
            llm=llm,
            tokenizer=tokenizer,
