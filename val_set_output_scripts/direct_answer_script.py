...
def generate_direct_batch(
    ...
    prompts = [build_prompt(tokenizer, q, DIRECT_PREFIX) for q in question_texts]
...
def main():
    parser = argparse.ArgumentParser(description="Collect direct-answer outputs with vLLM and cache them.")
    ...
    for batch_start in range(0, len(dataset), args.batch_size):
        ...
        batch_rows = generate_direct_batch(
            llm=llm,
            tokenizer=tokenizer,
